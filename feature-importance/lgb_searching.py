...
test_label_path = './input/validation/test_label.csv'
if not os.path.exists(test_label_path):
    # читаем исходный train.csv, чтобы взять таргеты
    full = pd.read_csv('./input/validation/train.csv', usecols=['msno','song_id','target'])
    # читаем тестовый фрейм (без таргета)
    test_tmp = pd.read_csv('./input/validation/test.csv', usecols=['msno','song_id'])
    # соединяем, чтобы получить таргет для каждой пары (msno, song_id)
    labels = (test_tmp
              .merge(full, on=['msno','song_id'], how='left')
              ['target']
             )
    # сохраняем
    os.makedirs(os.path.dirname(test_label_path), exist_ok=True)
    pd.DataFrame({'target': labels}).to_csv(test_label_path, index=False)
...
train_add = pd.read_csv('./input/validation/train_part_add.csv')

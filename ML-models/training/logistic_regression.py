...
# уменьшаем датасет
idx_train = train.sample(frac=0.5, random_state=42).index
train = train.loc[idx_train]
train_y = train_y.loc[idx_train]

# (опционально) то же для теста, если вы хотите ускорить и предикт
idx_test = test.sample(frac=0.5, random_state=42).index
test = test.loc[idx_test]
test_id = test_id.loc[idx_test]

print(f'[X] Reduced train to {train.shape[0]} rows, test to {test.shape[0]} rows')


#####################################################
## 8) Train LogisticRegression with logs
#####################################################
print('[8] Training LogisticRegression…')
...
# on train
pred_train = lr_clf.predict_proba(train)[:,1]
...
print(f'    train F1       = {f1_score(train_y, pred_train_bin):.5f}')
...
pred_test = lr_clf.predict_proba(test)[:,1]
flag      = np.random.randint(0,65536)
out       = pd.DataFrame({'id': test_id, 'target': pred_test})
fname     = f'{SUBMIT_DIR}/lr_{roc_auc_score(train_y,pred_train):.5f}_{flag}.csv.gz'

...
#####################################################
## 8) Train KNeighborsClassifier with logs
#####################################################
print('[8] Training KNeighborsClassifier…')
...
prob_train = knn.predict_proba(train)[:,1]
...
prob_test = knn.predict_proba(test)[:,1]
flag      = np.random.randint(0,65536)
out       = pd.DataFrame({'id': test_id, 'target': prob_test})
fname     = f'{SUBMIT_DIR}/knn_{roc_auc_score(train_y,prob_train):.5f}_{flag}.csv.gz'

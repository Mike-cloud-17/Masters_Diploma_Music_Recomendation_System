...
#####################################################
## 8) Train MultinomialNB with logs
#####################################################
print('[8] Training MultinomialNB…')
mnb = MultinomialNB()
t0 = time.time()
...
prob_train = mnb.predict_proba(train)[:,1]
...
model_path = os.path.join(MODEL_DIR, 'mnb_model.joblib')
joblib.dump(mnb, model_path)
print(f'    model saved to {model_path}')

print('[10] Predicting on test & saving submission…')
prob_test = mnb.predict_proba(test)[:,1]
flag      = np.random.randint(0,65536)
out       = pd.DataFrame({'id': test_id, 'target': prob_test})
fname     = f'{SUBMIT_DIR}/mnb_{roc_auc_score(train_y,prob_train):.5f}_{flag}.csv.gz'

...
#####################################################
## 8) Train RBF SVM with logs
#####################################################
print('[8] Training RBF SVC…')
svm = SVC(kernel='rbf', probability=True, cache_size=2048, verbose=False)
t0 = time.time()
...
prob_train = svm.predict_proba(train)[:,1]
...
model_path = os.path.join(MODEL_DIR, 'svm_rbf_model.joblib')
joblib.dump(svm, model_path)
print(f'    model saved to {model_path}')

print('[10] Predicting on test & saving submission…')
prob_test = svm.predict_proba(test)[:,1]
flag      = np.random.randint(0,65536)
out       = pd.DataFrame({'id': test_id, 'target': prob_test})
fname     = f'{SUBMIT_DIR}/svm_rbf_{roc_auc_score(train_y,prob_train):.5f}_{flag}.csv.gz'

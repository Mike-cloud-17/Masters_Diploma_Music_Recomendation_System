...
#####################################################
## 8) Train Decision Tree with logs
#####################################################
print('[8] Training DecisionTreeClassifier…')
...
# on train
pred_train = dt.predict_proba(train)[:,1]
...
pred_test = dt.predict_proba(test)[:,1]
flag      = np.random.randint(0,65536)
out       = pd.DataFrame({'id': test_id, 'target': pred_test})
fname     = f'{SUBMIT_DIR}/dt_{roc_auc_score(train_y,pred_train):.5f}_{flag}.csv.gz'
